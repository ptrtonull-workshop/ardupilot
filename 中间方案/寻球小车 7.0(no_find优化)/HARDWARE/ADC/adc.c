...
u16 Get_Adc_Average(u8 ch,u8 times)
{
	...
	//冒泡排序
	for(i=0;i<h-1;i++)   //n个数的数列总共扫描n-1次
    {  
        for(j=0;j<h-i-1;j++)//每一趟扫描到data[n-i-2]与data[n-i-1]比较为止结束  
        {  
            if(addata[j]>addata[j+1])//后一位数比前一位数小的话，就交换两个数的位置（升序）
            {  
               t=addata[j+1];  
               addata[j+1]=addata[j];  
               addata[j]=t;  
		...
		temp_val = (addata[41] + addata[42] + addata[43] + addata[44] + addata[45] + addata[46] + addata[47] + addata[48]);
    return temp_val/8;
//	u8 t;
//	for(t=0;t<times;t++)
//	{
...
u16 Get_Voltage_Average(u8 ch,u8 times)
{
//	u32 addata[10]={0};
	u32 temp_val=0;
	u8 t=0;
