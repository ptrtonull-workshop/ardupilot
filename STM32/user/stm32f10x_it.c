...
void TIM5_IRQHandler(void)//TIM5中断函数
{
}
...
{
	static u8 Redata;
	USART_ClearFlag(USART1,USART_FLAG_TC);//清除串口1发送完成标志位
	if(USART_GetITStatus(USART1,USART_IT_RXNE)!=Bit_RESET)//是否接收中断
	{
		Redata=USART_ReceiveData(USART1);//接收数据
		USART_ClearITPendingBit(USART1,USART_IT_RXNE);//清除中断
		while(USART_GetFlagStatus(USART1,USART_FLAG_RXNE)==RESET);//等待接收完成
		USART_SendData(USART1,Redata);//发送数据
